import {removeDotSegments, resolve} from "../lib/Resolve";
...
describe('#resolve', () => {
  ...
  it('create an IRI from a baseIRI and .', () => {
    expect(resolve('.', 'http://aa/'))
      .toEqual('http://aa/');
  });

  it('create an IRI from a baseIRI and ..', () => {
    expect(resolve('..', 'http://aa/b/'))
      .toEqual('http://aa/');
  });
  ...
  it('create an IRI from a baseIRI without ending slash and ?a=b', () => {
    expect(resolve('?a=b', 'http://abc/def/ghi'))
      .toEqual('http://abc/def/ghi?a=b');
  });

  it('create an IRI from a baseIRI without ending slash and .?a=b', () => {
    expect(resolve('.?a=b', 'http://abc/def/ghi'))
      .toEqual('http://abc/def/?a=b');
  });

  it('create an IRI from a baseIRI without ending slash and ..?a=b', () => {
    expect(resolve('..?a=b', 'http://abc/def/ghi'))
      .toEqual('http://abc/?a=b');
  });

  it('create an IRI from a baseIRI without ending slash and xyz', () => {
    expect(resolve('xyz', 'http://abc/d:f/ghi'))
      .toEqual('http://abc/d:f/xyz');
  });
...
describe('#removeDotSegments', () => {
  ...
  it('should handle another inbetween parent directory', () => {
    expect(removeDotSegments('mid/content=5/../6'))
      .toEqual('/mid/6');
  });
